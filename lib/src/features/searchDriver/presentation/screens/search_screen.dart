...
import '../../../../di/di.dart';
import '../../../homeDriver/presentation/widgets/list_items.dart';
...
class _SearchDriverScreenState extends State<SearchDriverScreen>
  ...
  Widget build(BuildContext context) {
    return MultiBlocProvider(
      providers: [BlocProvider(create: (_) => sl<DriverSearchTripsBloc>())],
      child: Scaffold(
        backgroundColor: _DriverRegionsSearchTab._bg,
        appBar: AppBar(
          ...
          title: Text(
            ...
            style: const TextStyle(
              fontSize: 18,
              color: _DriverRegionsSearchTab._text,
            ...
            child: Padding(
              ...
              child: Container(
                ...
                decoration: BoxDecoration(
                  ...
                  border: Border.all(color: _DriverRegionsSearchTab._border),
                ),
                child: TabBar(
                  controller: _tab,
                  ...
                  indicator: BoxDecoration(
                    color: _DriverRegionsSearchTab._softPrimary,
                    borderRadius: BorderRadius.circular(14),
                  ),
                  labelColor: _DriverRegionsSearchTab._primary,
                  unselectedLabelColor: _DriverRegionsSearchTab._muted,
        ...
        body: TabBarView(
          ...
          controller: _tab,
          children: [
            const _DriverRegionsSearchTab(),
            BlocProvider(
              create: (_) => sl<DriverCitySearchBloc>(),
              child: const DriverCitySearchTab(),
            ),
          ],
        ),
      ),
...
class _DriverRegionsSearchTab extends StatefulWidget {
  const _DriverRegionsSearchTab();
  ...
  @override
  State<_DriverRegionsSearchTab> createState() => _DriverRegionsSearchTabState();
}

class _DriverRegionsSearchTabState extends State<_DriverRegionsSearchTab>
    with AutomaticKeepAliveClientMixin {
  bool _filtersExpanded = true;

  String _from = 'Toshkent';
  ...
  void _onScroll() {
    if (_scrollController.position.pixels >=
      ...
      context.read<DriverSearchTripsBloc>().add(DriverSearchTripsLoadMoreRequested());
    }
  }
            ...
            children: [
              ...
              InkWell(
                ...
                child: Container(
                  ...
                  width: 56,
                  decoration: BoxDecoration(
                    color: _DriverRegionsSearchTab._bg,
                    borderRadius: BorderRadius.circular(18),
                    border: Border.all(color: _DriverRegionsSearchTab._border),
                  ),
                  alignment: Alignment.center,
                  child: const Icon(Icons.swap_horiz, color: _DriverRegionsSearchTab._primary),
                ),
              ),
            ],
          ),
  ...
  Widget _dateField(BuildContext context) {
    return InkWell(
      ...
      child: Container(
        ...
        decoration: BoxDecoration(
          color: _DriverRegionsSearchTab._fieldBg,
          borderRadius: BorderRadius.circular(18),
          border: Border.all(color: _DriverRegionsSearchTab._border),
        ),
        child: Row(
          children: [
            Expanded(
              child: Column(
                ...
                children: [
                  Text('search_date'.tr(),
                      style: const TextStyle(fontSize: 12, color: _DriverRegionsSearchTab._muted)),
                  const SizedBox(height: 3),
                  Text(
                    _date == null ? 'search_date_not_selected'.tr() : _formatDate(_date!),
                    style: const TextStyle(fontSize: 16, color: _DriverRegionsSearchTab._text),
                  ),
                ],
              ),
            ),
            if (_date != null)
              InkWell(
                ...
                child: const Padding(
                  ...
                  child: Icon(Icons.close, size: 18, color: _DriverRegionsSearchTab._muted),
                ),
              )
            else
              const Icon(Icons.keyboard_arrow_down, color: _DriverRegionsSearchTab._muted),
          ],
        ),
      ),
    );
  ...
  Widget _actionRow(BuildContext context) {
    return Row(
      children: [
        ...
        SizedBox(
          height: 50,
          child: BlocBuilder<DriverSearchTripsBloc, DriverSearchTripsState>(
            builder: (context, state) {
              final loading = state is DriverSearchTripsLoading;
              return ElevatedButton(
                onPressed: loading ? null : _runSearch,
                style: ElevatedButton.styleFrom(
                  elevation: 0,
                  backgroundColor: _DriverRegionsSearchTab._primary,
  ...
  Widget _summaryChip() {
    ...
    return Container(
      ...
      child: Text(
        ...
        style: const TextStyle(fontSize: 15, color: _DriverRegionsSearchTab._text),
      ),
    );
  }

  Widget _results(BuildContext context) {
    return BlocBuilder<DriverSearchTripsBloc, DriverSearchTripsState>(
      builder: (context, state) {
        if (state is DriverSearchTripsLoading) {
          return _surface(
            padding: const EdgeInsets.all(14),
            child: const Center(child: CircularProgressIndicator()),
          );
        }
        if (state is DriverSearchTripsError) {
          return _surface(
            padding: const EdgeInsets.all(14),
        ...
        if (state is DriverSearchTripsLoaded) {
          ...
          return Column(
            children: [
              ...trips.map((t) => TripCard.passengerTrip(t, () {
                if (actionLoading) return;
                context.read<DriverSearchTripsBloc>().add(
                  DriverSearchCreateBookingRequested(tripId: t.id),
                );
              })),
              ...
              if (hasMore) ...[
                ...
                if (paginationLoading)
                  ...
                  SizedBox(
                    ...
                    child: OutlinedButton(
                      onPressed: () => context
                          .read<DriverSearchTripsBloc>()
                          .add(DriverSearchTripsLoadMoreRequested()),
                      style: OutlinedButton.styleFrom(
                        side: const BorderSide(color: _DriverRegionsSearchTab._border),
                        foregroundColor: _DriverRegionsSearchTab._primary,
                ...
                Center(
                  child: Text(
                    ...
                    style: const TextStyle(
                      fontSize: 12,
                      color: _DriverRegionsSearchTab._muted,
                    ),
                  ),
                ),
              ],
          ...
          children: [
            Expanded(
              child: Column(
                crossAxisAlignment: CrossAxisAlignment.start,
                children: [
                  Text(label, style: const TextStyle(fontSize: 12, color: _DriverRegionsSearchTab._muted)),
                  const SizedBox(height: 3),
                  Text(value, style: const TextStyle(fontSize: 16, color: _DriverRegionsSearchTab._text)),
                ],
              ),
            ),
            const Icon(Icons.keyboard_arrow_down, color: _DriverRegionsSearchTab._muted),
  ...
  void _runSearch() {
    ...
    context.read<DriverSearchTripsBloc>().add(
      DriverSearchTripsRequested(from: _from, to: _to, date: dateStr),
    );
  }
                ...
                children: [
                  Padding(
                    ...
                    child: Row(
                      children: [
                        Expanded(
                          child: Text(title, style: const TextStyle(fontSize: 16, color: _DriverRegionsSearchTab._text)),
                        ),
                        TextButton(
                  ...
                  Padding(
                    ...
                    child: TextField(
                      ...
                      decoration: InputDecoration(
                        ...
                        filled: true,
                        fillColor: _DriverRegionsSearchTab._bg,
                      ...
                      builder: (_, list, __) {
                        return ListView.separated(
                          ...
                          itemBuilder: (_, i) {
                            ...
                            return ListTile(
                              title: Text(city, style: const TextStyle(fontSize: 15, color: _DriverRegionsSearchTab._text)),
                              trailing: isSel ? const Icon(Icons.check, color: _DriverRegionsSearchTab._primary) : null,
