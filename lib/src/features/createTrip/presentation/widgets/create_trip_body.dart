...
class IntercityTripForm extends StatelessWidget {
  const IntercityTripForm({super.key});

  @override
  Widget build(BuildContext context) {
    return BlocProvider(
      create: (_) => sl<TripCreateBloc>(),
      child: const _Body(),
    );
  }
}
    ...
    return BlocConsumer<TripCreateBloc, TripCreateState>(
      ...
      listener: (context, s) async {
        ...
        if (s.status == TripCreateStatus.failure) {
          await _showErrorFlushbar(context, "msg_error_occurred".tr());
        }
      },
      builder: (context, s) {
            ...
            ListView(
              ...
              children: [
                ...
                _SoftSelectField(
                  ...
                  enabled: !loading,
                  onTap: () async {
                    final picked = await _pickPassengersBottomSheet(
                      context,
                      initial: s.seats,
                      min: 1,
                      max: 4,
                    ...
                    context
                        .read<TripCreateBloc>()
                        .add(TripCreateSeatsChanged(picked));
                  },
                ),
                const SizedBox(height: 12),
                _SoftMoneyField(
              ...
              child: _StickyFooter(
                child: SizedBox(
                  ...
                  child: ElevatedButton(
                    ...
                    child: Text(loading ? 'btn_creating'.tr() : 'btn_create_trip'.tr()),
                  ),
                ),
              ),
            ),
        ...
        child: Row(
          children: [
            ...
            Icon(
              ...
              size: 24,
              color: enabled ? const Color(0xFF8B95A7) : const Color(0xFFBDBDBD),
            ),
          ],
        ),
      ),
...
class _SoftMoneyFieldState extends State<_SoftMoneyField> {
  ...
  Widget build(BuildContext context) {
    final bg =
    widget.enabled ? const Color(0xFFF5F7FF) : const Color(0xFFF3F4F6);
    final border =
    widget.enabled ? const Color(0xFFE6ECFF) : const Color(0xFFE5E7EB);
...
Future<int?> _pickPassengersBottomSheet(
    BuildContext context, {
            ...
            child: StatefulBuilder(
              builder: (context, setLocal) {
                return Row(
                  children: [
                    IconButton(
                      onPressed: temp > min ? () => setLocal(() => temp--) : null,
                    ...
                    IconButton(
                      onPressed: temp < max ? () => setLocal(() => temp++) : null,
