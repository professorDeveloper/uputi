...
class DriverHistoryResponse {
  ...
  DriverHistoryResponse({required this.trips});
  ...
  factory DriverHistoryResponse.fromJson(Map<String, dynamic> json) {
    ...
    return DriverHistoryResponse(
      trips: isBookingResponse
          ...
          : TripsPage.fromTripsJson(pageMap),
    );
  }
}
...
class TripsPage {
  ...
  /// type=1: data elementi to'g'ridan Trip
  factory TripsPage.fromTripsJson(Map<String, dynamic> json) {
    return TripsPage(
      ...
      data: (json['data'] as List? ?? [])
        ...
        try {
          ...
          debugPrint('[DriverHistory] Trip parse error: $err');
          return null;
        }
      })
          .whereType<Trip>()
  ...
  factory TripsPage.fromBookingsJson(Map<String, dynamic> json) {
    final trips = (json['data'] as List? ?? [])
      ...
      try {
        ...
        // booking status ni trip ga override qilamiz
        return Trip.fromJson(tripMap, bookingOverride: bookingMap);
      } catch (err) {
        debugPrint('[DriverHistory] Booking->Trip parse error: $err | data: $e');
        return null;
      }
    })
        .whereType<Trip>()
  ...
  static List<PageLink> _parseLinks(dynamic raw) {
    return (raw as List? ?? [])
...
class Trip {
  ...
  /// type=2: trip yaratgan yo'lovchi (trip.user)
  final User? tripOwner;

  /// type=1: trip ichidagi bookings
  ...
  factory Trip.fromJson(
      Map<String, dynamic> json, {
        Map<String, dynamic>? bookingOverride,
      }) {
    // type=2: booking.status ishlatiladi
    final statusStr = bookingOverride != null
        ? (bookingOverride['status'] ?? json['status'] ?? '').toString()
        : (json['status'] ?? '').toString();

    // trip.user — trip yaratgan yo'lovchi (type=2 da keladi)
    User? owner;
    if (json['user'] != null) {
      try {
...
class User {
  ...
  factory User.empty() => User(id: 0, name: '', phone: '', role: '');
}
