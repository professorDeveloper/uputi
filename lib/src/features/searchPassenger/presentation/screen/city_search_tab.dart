...
class CitySearchTab extends StatefulWidget {
  const CitySearchTab({super.key});

  @override
  State<CitySearchTab> createState() => _CitySearchTabState();
}

class _CitySearchTabState extends State<CitySearchTab> {
  ...
  Future<void> _load() async {
    ...
    context.read<CitySearchBloc>().add(
      CitySearchRequested(lat: _lastLat, lng: _lastLng),
    );
  ...
  Future<void> _openTripSheet(CityTripItem t) async {
    ...
    final tripsBloc = parentContext.read<SearchTripsBloc>();
  ...
  Future<Uint8List> _buildCarMarkerBytes({
...
class _CityTripBottomSheet extends StatelessWidget {
  final CityTripItem trip;
  final VoidCallback onBook;
  final VoidCallback onOffer;
  final bool actionLoading;

  const _CityTripBottomSheet({
    required this.trip,
...
class _SheetMiniInfo extends StatelessWidget {
  final String label;
  final String value;

  const _SheetMiniInfo({required this.label, required this.value});
