...
import 'package:uputi/src/features/auth/domain/entities/user_role.dart';
...
class Prefs {
  ...
  static Future<bool?> setUserPhone(String value) async =>
      await _prefs?.setString("userPhone", value);

  static Future<bool?> setUserPassword(String value) async =>
      await _prefs?.setString("userPassword", value);
  ...
  static String? getStaffID() => _prefs?.getString("staffID");
  ...
  static Future<void> logout() async {
    ...
    await p.remove("role");
    await p.remove("userPhone");
    await p.remove("userPassword");
  }
}
